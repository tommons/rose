...
void loop() {
  ...
  //later.
  control = dmx_slave.getChannelValue (DMX_ADDRESS);
  red = dmx_slave.getChannelValue (DMX_ADDRESS+1);
  //Green is our second, so take the start channel and add 1 to it. Same for
  //blue and intensity.
  green = dmx_slave.getChannelValue (DMX_ADDRESS + 2);
  blue = dmx_slave.getChannelValue (DMX_ADDRESS + 3);

/*
  if( control != control1 || red != red1 || blue != blue1 || green != green1 )
  {
    Serial.print("new ");
    ...
    Serial.print(red1); Serial.print(" "); 
    Serial.print(blue1); Serial.print(" "); 
...
*/
  ...
  if( elapsed > 100 )
  {
      ...
      Wire.write(control);              // sends one byte  
      Wire.write(red);              // sends one byte  
      Wire.write(green);              // sends one byte  
      Wire.write(blue);              // sends one byte  
